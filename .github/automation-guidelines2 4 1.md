...
- Agrupa features por proyecto en `src/test/resources/features`.
...
## 6. karate-config.js base (`src/test/resources/karate-config.js`)
...
Feature: BTPMCDP-118 Crear cuenta exitosamente (microservicio transversal para crear cuentas)
  Background:
    ...
    * def headers = generarHeaders()
    * headers headers
  @id:1 @crearCuentaKyc @solicitudExitosa200  Scenario: T-API-BTPMCDP-118-CA01-Crear cuenta exitosamente 201 - karate
    * def jsonData = read('classpath:data/tre_msa_savings_account/request_creation_account.json')
    * def result = call read('classpath:features/utility/generar_compliance_id.feature@generar_compliance_id')
...
- **Ubicación de Karate config**: `src/test/resources/karate-config.js`
...
2. **Verificación de karate-config.js**:
   - Confirmar que el archivo existe en `src/test/resources/karate-config.js`
